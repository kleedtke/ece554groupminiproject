...
int main(int argc, char *argv[]) {
  ...
  try {
		...
		for (int j = 0; j<DIM_FULL; j+=8) { // iterate over block cols of C
			...
			for (int k = 0; k<DIM_FULL; k+=8) { // iterate over block products
				...
				if (clock_gettime(CLOCK_MONOTONIC, &end_compute)!=0) // error check
				      cerr << "ERROR: Couldn't clock_gettime()." << endl;
				total_compute += getTime(start_compute,end_compute);// (end_compute - start_compute)
	...
	total_time = getTime(start,end); // end - start
	ops_rate = 2*(DIM_FULL*DIM_FULL*DIM_FULL) / total_time;  // MM is O(n^3) MACs, each MAC is 2 ops
	compute_ops_rate = 2*(DIM_FULL*DIM_FULL*DIM_FULL) / total_compute; // TOPS ignoring data movement
	tops_rate = ops_rate / (1000000000000); // 10^12
	compute_tops_rate = compute_ops_rate / (1000000000000); // 10^12

	fprintf(stdout, "For DIM_FULL=%d\n",DIM_FULL);
